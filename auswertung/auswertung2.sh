#!/bin/bash
echo 'Evaluation-Script for experiments by C.F.Joern: auswertung.sh v. 2.0'
echo 'please give number of experiment:'
read foldername
mkdir $foldername
cd $foldername
echo 'copy the prepared file into the new folder, then save it as csv'
echo 'when done, give the files FULL name (example.csv):'
read filename
